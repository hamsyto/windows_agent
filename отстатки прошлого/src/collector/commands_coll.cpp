...
double GetTotalDisk(const std::string& root) {
  ...
  return round((static_cast<double>(totalBytes.QuadPart) / (1024.0 * 1024.0)) *
               100.0) /
         100.0;
}
...
double GetUsedDisk(const std::string& root) {
  ...
  return round((static_cast<double>(totalBytes.QuadPart - freeBytes.QuadPart) /
                (1024.0 * 1024.0)) *
               100.0) /
         100.0;
}
...
int GetPhysicalDiskIndexForDriveLetter(char letter) {
  ...
  hres = pSvc->ExecQuery(const_cast<LPWSTR>(L"WQL"), query,
  ...
  if (SUCCEEDED(hres) && pEnum) {
    ...
    ULONG uReturn = 0;
    if (pEnum->Next(WBEM_INFINITE, 1, &pPart, &uReturn) == S_OK && uReturn) {
      VARIANT vIndex;
      VariantInit(&vIndex);
...
string GetComputerDomainOrWorkgroup() {
  ...
  if (status == NERR_Success && domainName != nullptr) {
    ...
    if (len > 0) {
      ...
      WideCharToMultiByte(CP_ACP, 0, domainName, -1, &temp[0], len, nullptr,
                          nullptr);
      result = move(temp);
    }
