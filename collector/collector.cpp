...
vector<Disk> GetDisks() {
  ...
  for (char letter = 'A'; letter <= 'Z'; ++letter) {
    ...
    if (diskIndex < 0) continue;  // если не удалось определить физический диск
...
RAM GetRam() {
  ...
  ram.total = round(ram.total / (1024 * 1024) * 100.0) / 100.0;
  ram.used = round(ram.used / (1024 * 1024) * 100.0) / 100.0;
