...
++ обрезать float;
2. память не free, а usage
3. список, у mac-адресов может быть ip, а может и не быть. но если есть, нужно его (ip) прислать (вид: macадрес ipадрес (через пробел))
отдельная структура для флешок
?4. починить диски. usb добавить, когда их нет, просто не отправлять инфу о них
5. написать сжималку json
6. написать шифровальщик bytes
