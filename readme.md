...
++ обрезать float;
0. когда подключаю флешку он просто дублирует логические диски, физ диска 0. лажа :(
1. везде убрать float, заменить на uint
...
10. доработка класса, чтобы работающие в нём функции стали чисто методами класса

11. Добавить отдельный тип сообщения IAmNotOK, если произошла какая-то критическая ошибка, например — ты не смогла собрать какие-то данные или что-то непонятное произошло. Например — нет разрешений на сбор данных каких-то, не смогла сохранить ID в файл. Если произошла ошибка, посылаешь подобный header, а в сообщении просто string "text" в котором описание ошибки
