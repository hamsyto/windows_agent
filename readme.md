...
- везде убрать float, заменить на uint
- сделать dll файл. он будет лежать рядом с exe (json.hpp нужно в dll превратить)
- (потом прям) папку src. там всё кроме main.cpp


нужно сделать (новое!):
- winConnection.cpp вообще нужен?
- SendMessageAndMessageSize() не send а класс connection
...
переименновать в windowsConnection

на сегодня:
+ почистить импорты в транспорт
2. посмотреть, надо ли нам то говно, что нагенерил квен (в подпапках), если нет, удаляй; (но может понадобиться декрипт (расшифровать))
3. Ебануть инкапсуляцию работы с сокетом в connection
3.1 ВСЮ логику в сокетом закинуть в него, он сам будет отвечать за состояние соединения, обрабатывать ошибки отправки и тд.
3.2 У него будет 2 метода: послать байтики, запросить байтики. (send, recv буквально)
3.3 ИЗВНЕ ты не меняешь состояние "connected", это свойство самого connection

по 3, я хочу, я НАДЕЮСЬ что это возможно, чтобы у тебя импорт (win/winsocket чисто виндовых библиотек с сокетом) был только в 1 классе/модуле, угу?
