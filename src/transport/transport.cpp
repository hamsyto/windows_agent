...
void Transport::SendData(const std::string& jsonStr) {
    ...
    m_connection.Send(header + encrypted);
}
...
std::vector<unsigned char> Transport::DeriveKey(const std::string& rawKey) {
    ...
    SHA256(reinterpret_cast<const unsigned char*>(rawKey.c_str()),
           rawKey.size(), hash.data());
    return hash;
}
...
std::string Transport::Encrypt(const std::string& data) {
    ...
    // Выделяем буфер достаточного размера (для ChaCha20 = data.size(), но
    // делаем с запасом)
    std::vector<unsigned char> ciphertext_buf;
    ciphertext_buf.resize(
        data.size() +
        EVP_MAX_BLOCK_LENGTH);  // хотя для ChaCha EVP_MAX_BLOCK_LENGTH = 1
    ...
    int final_len = 0;
    if (1 != EVP_EncryptFinal_ex(ctx, nullptr, &final_len)) {
        EVP_CIPHER_CTX_free(ctx);
    ...
    // Формируем результат: nonce (12) + ciphertext (len) + tag (16)
    std::string result;
    result.reserve(NONCE_SIZE + len + TAG_SIZE);
    result.append(reinterpret_cast<const char*>(nonce.data()), NONCE_SIZE);
    result.append(reinterpret_cast<const char*>(ciphertext_buf.data()), len);
